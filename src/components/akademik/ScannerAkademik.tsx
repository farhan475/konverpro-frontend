...
type ScanResult = {
  mk_asal: string;
  sks_asal: string | number;
  nilai_asal: string | number;
  id_mk_tujuan: number | null;
  mk_tujuan: string;
  sks_tujuan: number;
  diakui: boolean;
};
    ...
    reader.onload = (e) => {
      try {
        ...
        const prodi = prodiData.find(
          (p) => p.id.toString() === selectedProdiId,
        );
        const kurikulum = prodi?.kurikulum || [];

        const scanResults: ScanResult[] = jsonData.map((row) => {
          const mkAsal = String(
            row["Nama Mata Kuliah"] || row["Mata Kuliah"] || row["MK"] || "",
          );
          const sksAsal = Number(row["SKS"] || 0);
          const nilaiAsal = String(row["Nilai"] || "");

          let bestMatch: typeof kurikulum = [];
          let bestScore = 0;
          ...
          return {
            ...
            id_mk_tujuan: bestScore > 0.6 ? bestMatch[0]?.id : null,
            mk_tujuan: bestScore > 0.6 ? bestMatch[0]?.nama_mk : "-",
            sks_tujuan: bestScore > 0.6 ? bestMatch[0]?.sks : 0,
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/akademik/scanner/save`,
        {
          method: "POST",
      ...
      <div className="lg:col-span-4 space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          ...
          >
            ...
            <CloudArrowUp
              size={32}
              className="text-slate-400 group-hover:text-blue-600 transition mb-2"
            />
            ...
            <div className="p-5 border-t border-slate-100 bg-white">
              ...
              >
                {saving ? (
                  ...
                  <>
                    <PaperPlaneRight size={18} /> Simpan & Teruskan
                  </>
                )}
              </button>
            </div>
