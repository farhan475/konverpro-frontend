...
export default function ManajemenUser() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.success) {
        ...
        toast.success(
          editingUser
            ? "Informasi user berhasil diperbarui"
  ...
  const handleDelete = async (id: number, name: string) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/admin-pt/users/${id}`,
        { method: "DELETE" },
      );
      const json = await res.json();
      if (json.success) {
    ...
    <div className="space-y-8">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="border-l-4 border-[#FDD824] pl-5">
          ...
          <h2 className="font-heading text-2xl lg:text-3xl font-black text-[#031f37] mt-1 uppercase tracking-tight">
            MANAJEMEN USER
          </h2>
          <p className="mt-1.5 text-sm text-slate-500 font-medium">
            Kelola akses akun Staf Akademik, Staff Intake, dan Kaprodi.
          </p>
        </div>
        <button
                ...
                >
                  <td className="py-5 px-6">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="font-black text-[#031f37] uppercase text-xs tracking-tight">
                        {user.nama_lengkap}
                      </p>
                    </div>
                  </td>
                  <td className="py-5 px-4">
                    <div className="space-y-1">
                      <p className="flex items-center gap-1.5 text-slate-600 font-bold">
                        <Envelope size={14} className="text-slate-300" />{" "}
                        {user.email}
                      </p>
                      <p className="flex items-center gap-1.5 text-[10px] text-slate-400 font-bold">
                        <WhatsappLogo size={14} className="text-emerald-500" />{" "}
                        {user.no_whatsapp || "-"}
                      </p>
                    </div>
                  </td>
                  <td className="py-5 px-4 text-center">
                    <span className="px-3 py-1 bg-blue-50 text-blue-800 rounded-lg font-black text-[10px] uppercase border border-blue-100">
                  ...
                  <td className="py-5 px-4 text-center">
                    {user.role === "kaprodi" ? (
                      <span className="text-[10px] font-black text-[#031f37] bg-yellow-50 px-2 py-0.5 rounded-full border border-yellow-200">
                        {user.total_prodi_dipegang} PRODI
                      </span>
                    ) : (
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-[#031f37]/60 backdrop-blur-sm animate-in fade-in duration-300"
          ...
          <div className="relative w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 fade-in duration-300">
            <div className="bg-[#031f37] p-8 text-white flex items-center justify-between">
              <div>
                ...
                <h3 className="font-heading text-xl font-black mt-1 uppercase tracking-tight">
                  {editingUser
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, no_whatsapp: e.target.value })
                    }
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="akademik">
                      Staf Akademik / Verifikator
                    </option>
                    <option value="staff">
                      Staff Intake / Admin Pendaftaran
                    </option>
                  </select>
                </div>
                <div>
