...
export default function KamusSinonimPage() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const { data } = await api.get<ApiResponse<KamusSinonim[]>>('/api/superadmin/kamus-sinonim');
      if (data.success) {
        setItems(data.data);
      }
    } catch (error) {
  ...
  const handleSaveItem = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const { data } = await api.post('/api/superadmin/kamus-sinonim', formData);
      if (data.success) {
  ...
  const handleDeleteItem = async (id: string) => {
    ...
    try {
      const { data } = await api.delete(`/api/superadmin/kamus-sinonim/${id}`);
      if (data.success) {
