...
export default function LaporanGlobalPage() {
  const [data, setData] = useState<SuperadminLaporan | null>(null);
  const [loading, setLoading] = useState(true);

  const fetchLaporan = async () => {
    setLoading(true);
    try {
      const { data } = await api.get<ApiResponse<SuperadminLaporan>>('/api/superadmin/laporan');
      if (data.success) {
        setData(data.data);
      }
    } catch {
  ...
  const prodiColumns = [
    ...
    },
    {
      ...
      accessor: 'total_mhs' as keyof ProdiReportRow,
    },
    {
      header: 'Disetujui',
      className: 'text-center',
      accessor: 'approved' as keyof ProdiReportRow,
    },
    {
      header: 'SKS Diakui',
      className: 'text-center',
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard label="Total Pendaftar" value={data?.global.total_pendaftar || 0} icon={Users} variant="blue" />
        <StatCard label="Total SKS Diakui" value={data?.global.total_sks_diakui || 0} icon={FilePdf} variant="green" />
        <StatCard label="Rata-rata SKS / Mhs" value={data?.global.avg_sks_per_mhs || 0} icon={TrendUp} variant="purple" />
        <StatCard label="Total Prodi" value={data?.by_prodi.length || 0} icon={Buildings} variant="yellow" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <Card>
            ...
            <DataTable<ProdiReportRow> columns={prodiColumns} data={data?.by_prodi || []} loading={loading} />
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card>
            ...
            <div className="space-y-4">
              {data?.by_status.map((s: StatusSummary) => (
          ...
          <Card className="bg-blue-900 text-white border-none">
            ...
            <div className="space-y-4">
              {data?.monthly_trends.map((t: MonthlyTrend) => (
