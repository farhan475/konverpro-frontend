...
export default function SuperadminDashboard() {
  ...
  if (loading)
    return (
      <div className="flex items-center justify-center h-64">
        <Spinner className="animate-spin text-blue-600" size={32} />
      </div>
    );
  if (!data) return null;
  ...
  const cards = [
    {
      label: "Total Revenue",
      value: `Rp ${(data.stats.total_revenue / 1000000).toFixed(1)}M`,
      icon: Money,
    ...
    {
      label: "Pending Topup",
      value: data.stats.pending_topup,
      icon: CreditCard,
      color: "text-orange-600",
      bg: "bg-orange-50",
      pulse: data.stats.pending_topup > 0,
    },
    {
      label: "Global Approved",
      value: data.stats.total_mahasiswa_approved,
      icon: CheckCircle,
    ...
    <div className="space-y-8 pb-12">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <h2 className="font-heading text-3xl lg:text-4xl font-black text-slate-900 uppercase tracking-tight">
            KONTROL PUSAT
          </h2>
          <p className="mt-2 text-sm text-slate-500 font-medium">
            Monitoring performa platform KonverPro secara global.
          </p>
        </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {cards.map((card, i) => (
          <div
            key={i}
            className="akd-card p-6 flex items-center justify-between group hover:border-black transition-all"
          >
            <div>
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">
                {card.label}
              </p>
              <p className={cn("text-2xl font-black mt-1", card.color)}>
                {card.value}
              </p>
            </div>
            <div
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="akd-card p-6">
          <div className="flex items-center justify-between mb-8">
            <h3 className="font-heading text-sm font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
              <UsersThree size={20} weight="bold" className="text-blue-600" />{" "}
              Pertumbuhan Pendaftar
            </h3>
          </div>
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
                ...
                <Bar dataKey="total" radius={[6, 6, 0, 0]} barSize={40}>
                  {data.charts.registration.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
                        index === data.charts.registration.length - 1
                          ? "#031f37"
                          : "#3b82f6"
                      }
                    />
                  ...
                  {data.top_mitra.map((mitra) => (
                    ...
                    >
                      <td className="py-5 px-6">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 mt-0.5 uppercase tracking-tighter italic">
                          MITRA #{mitra.id.toString().padStart(4, "0")}
                        </p>
                      </td>
                      <td className="py-5 px-4 text-center">
                        <span
                          className={cn(
                            ...
                            mitra.paket_layanan === "Enterprise"
                      ...
                      <td className="py-5 px-6 text-right">
                        <Link
                          href={`/superadmin/mitra?id=${mitra.id}`}
          ...
          <div className="akd-card p-6 h-full flex flex-col">
            ...
            <div className="space-y-4 flex-1">
              {data.recent_activities.map((log) => (
                <div
                  key={log.id}
                  className="pb-4 border-b border-slate-50 last:border-0 last:pb-0 flex gap-3"
                >
            ...
            <Link
              ...
              className="mt-8 block w-full py-4 bg-slate-50 text-slate-400 rounded-2xl text-center text-[10px] font-black uppercase tracking-[0.2em] hover:bg-black hover:text-white transition-all"
            >
              Lihat Seluruh Log
            </Link>
          </div>
